-- Search menu items by name with offer-only filter
-- Param 1: search term (pass as '%term%' from PHP)
-- Param 2: offersOnly flag (0 or 1)
-- Cuisine filter and ORDER BY appended dynamically in PHP
SELECT
    mi.item_id,
    mi.item_name,
    mi.item_image,
...
WHERE mi.is_available = 1
  AND mi.item_name LIKE ?
  AND (? = 0 OR o.offer_id IS NOT NULL)