...
<div class="p-8">
        ...
        <div class="bg-white p-6 rounded shadow">
            ...
            <div class="space-y-3">
                @if(isset($activeOffers) && $activeOffers->count() > 0)
                    ...
                    <div class="flex justify-between items-center py-2 border-b">
                        <div>
                            ...
                            <p class="text-sm text-gray-500">{{ $offer->discount_value }}% off • Expires {{ \Carbon\Carbon::parse($offer->end_datetime)->format('M d') }}</p>
                        </div>
                ...
                @else
                    ...
                    <a href="{{ route('restaurant.add_offer') }}" class="text-orange-500 text-sm mt-2 inline-block">Create your first offer →</a>
                @endif
            </div>
        </div>
    ...
    <div class="grid md:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded shadow">
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <span class="bg-green-100 text-green-800 px-2 py-1 rounded text-xs">{{ $availableItems ?? $itemCount ?? 0 }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-sm">Unavailable Items</span>
                    <span class="bg-red-100 text-red-800 px-2 py-1 rounded text-xs">{{ $unavailableItems ?? 0 }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-sm">Items with Offers</span>
                    <span class="bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs">{{ $itemsWithOffers ?? 0 }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded shadow">
            ...
            <div class="space-y-3">
                <a href="{{ route('restaurant.add_item') }}" class="block w-full bg-orange-500 text-white text-center py-2 px-4 rounded hover:bg-orange-600">
                    + Add New Item
                </a>
                <a href="{{ route('restaurant.add_offer') }}" class="block w-full bg-blue-500 text-white text-center py-2 px-4 rounded hover:bg-blue-600">
                    Create Offer
                </a>
                <a href="{{ route('restaurant.items') }}" class="block w-full bg-gray-500 text-white text-center py-2 px-4 rounded hover:bg-gray-600">
                    Manage Menu
                </a>
                <a href="{{ route('restaurant.analytics') }}" class="block w-full bg-purple-500 text-white text-center py-2 px-4 rounded hover:bg-purple-600">
                    View Analytics
                </a>
            </div>
        </div>
