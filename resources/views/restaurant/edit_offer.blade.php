...
<head>
    ...
    <title>Edit Offer | {{ $restaurant->name ?? 'Restaurant' }}</title>
...
<body>
    ...
    <div class="page-wrap max-w-3xl mx-auto px-4 md:px-8 py-10 pb-20">
        ...
        <div class="mb-10 animate-in">
            <a href="{{ route('restaurant.offers') }}" class="back-link mb-6">
                <i data-feather="arrow-left" class="w-3.5 h-3.5"></i> Offers
            </a>
        ...
        <form id="statusToggleForm" action="{{ route('restaurant.toggleOfferStatus', $offer->offer_id) }}" method="POST" class="hidden">
            @csrf
        </form>

        <form action="{{ route('restaurant.updateOffer', $offer->offer_id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-card p-8 md:p-10 space-y-10 animate-in delay-2">
                ...
                <div class="space-y-6">
                    <div class="flex items-center justify-between">
                        <p class="section-eyebrow mb-0">Offer Details</p>
                        <div class="flex flex-col items-end">
                            <label class="field-label">Change Status</label>
                            <button type="button" onclick="document.getElementById('statusToggleForm').submit()"
                                class="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all duration-300 shadow-sm cursor-pointer
                                {{ $offer->is_active ? 'bg-emerald-50 text-emerald-600 border border-emerald-100 hover:bg-emerald-100' : 'bg-red-50 text-red-600 border border-red-100 hover:bg-red-100' }}">
                                @if($offer->is_active)
                    ...
                    <div>
                        ...
                        <input type="text" name="offer_title" value="{{ old('offer_title', $offer->offer_title) }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="select-wrap">
                                <select name="discount_type" id="discount_type"
                                    ...
                                    <option value="free_delivery" {{ old('discount_type', $offer->discount_type) == 'free_delivery' ? 'selected' : '' }}>Free Delivery</option>
                                </select>
                        ...
                        <div id="discount_value_container">
                            ...
                            <input type="number" name="discount_value" id="discount_value"
                                value="{{ old('discount_value', rtrim(rtrim($offer->discount_value, '0'), '.')) }}"
                        ...
                        <div>
                            ...
                            <div class="select-wrap">
                                <select name="target_type" id="target_type"
                                    ...
                                    <option value="restaurant" {{ old('target_type', $offer->target_type) == 'restaurant' ? 'selected' : '' }}>Entire Restaurant</option>
                                    <option value="category" {{ old('target_type', $offer->target_type) == 'category' ? 'selected' : '' }}>Specific Category</option>
                                    <option value="item" {{ old('target_type', $offer->target_type) == 'item' ? 'selected' : '' }}>Specific Item</option>
                                </select>
                                <i data-feather="chevron-down" class="w-4 h-4 chevron"></i>
                            </div>
                        </div>
                        ...
                        <div id="target_category_container" class="hidden">
                            ...
                            <div class="select-wrap">
                                <select name="target_category_id" class="field-input">
                                    ...
                                    <option value="{{ $cat->category_id }}" {{ old('target_category_id', $offer->target_category_id) == $cat->category_id ? 'selected' : '' }}>{{ $cat->category_name }}</option>
                                    @endforeach
                                </select>
                                <i data-feather="chevron-down" class="w-4 h-4 chevron"></i>
                            </div>
                        ...
                        <div id="target_item_container" class="hidden">
                            ...
                            <div class="select-wrap">
                                <select name="target_item_id" class="field-input">
                                    ...
                                    <option value="{{ $item->item_id }}" {{ old('target_item_id', $offer->target_item_id) == $item->item_id ? 'selected' : '' }}>{{ $item->item_name }}</option>
                                    @endforeach
                                </select>
                                <i data-feather="chevron-down" class="w-4 h-4 chevron"></i>
                            </div>
                    ...
                    <div>
                        ...
                        <div class="prefix-wrap">
                            ...
                            <input type="number" name="min_order_amount"
                                value="{{ old('min_order_amount', $offer->min_order_amount ? rtrim(rtrim($offer->min_order_amount, '0'), '.') : '') }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="datetime-local" name="start_date"
                                value="{{ old('start_date', date('Y-m-d\TH:i', strtotime($offer->start_datetime))) }}"
                                class="field-input" required>
                        </div>
                        <div>
                            <label class="field-label">Ends</label>
                            <input type="datetime-local" name="end_date"
                                value="{{ old('end_date', date('Y-m-d\TH:i', strtotime($offer->end_datetime))) }}"
                                class="field-input" required>
                        </div>
                    </div>
                </div>
