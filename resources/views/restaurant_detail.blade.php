...
<body class="bg-gray-50 relative font-sans text-gray-800">
    ...
    <nav class="sticky top-0 z-50 bg-white/90 backdrop-blur-md shadow-sm w-full py-4 px-6 sm:px-10 lg:px-20 flex justify-between items-center transition-all">
        <!-- Logo/Back -->
        <a href="{{ url('/') }}" class="text-xl font-black text-orange-500 tracking-tighter hover:scale-105 transition-transform inline-flex items-center">
            <i data-feather="arrow-left" class="w-5 h-5 mr-2"></i> Back
        </a>

        <!-- Cart Icon Wrapper -->
        <div class="relative group">
            <button id="cartBtn" class="relative p-2.5 bg-orange-50 rounded-full text-orange-500 hover:bg-orange-100 hover:shadow-lg transition-all focus:outline-none">
                <i data-feather="shopping-cart" class="w-6 h-6"></i>
                <span id="cartCount" class="absolute top-0 right-0 -mt-1 -mr-1 bg-red-500 text-white text-[10px] font-black px-1.5 py-0.5 rounded-full hidden">0</span>
            </button>

            <!-- Cart Dropdown (Visible on hover/click) -->
            <div id="cartDropdown" class="hidden absolute right-0 mt-3 w-80 sm:w-96 bg-white shadow-2xl rounded-2xl overflow-hidden border border-gray-100 z-[100] transform opacity-0 scale-95 transition-all duration-200 origin-top-right">
                <div class="p-5 bg-gradient-to-r from-orange-400 to-orange-500 text-white">
                ...
                <div class="p-5 border-t border-gray-100 bg-white shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)] relative z-10">
                    ...
                    <div class="flex gap-3">
                        <button id="clearCartBtn" class="flex-1 py-3 rounded-xl border-2 border-gray-200 text-gray-500 font-black hover:bg-gray-100 transition-colors text-sm uppercase tracking-wider">Clear</button>
                        <button id="checkoutBtn" class="flex-[2] py-3 rounded-xl border-2 border-orange-500 bg-orange-500 text-white font-black hover:bg-orange-600 transition-colors shadow-lg shadow-orange-200 text-sm uppercase tracking-wider">Checkout</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <header class="bg-white shadow-sm pt-8 pb-12 px-6 sm:px-10 lg:px-20 border-b border-gray-100 relative overflow-hidden">
        <!-- Background Pattern -->
        <div class="absolute inset-0 opacity-[0.03] pointer-events-none" style="background-image: url('data:image/svg+xml,%3Csvg width=\'60\' height=\'60\' viewBox=\'0 0 60 60\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cg fill=\'none\' fill-rule=\'evenodd\'%3E%3Cg fill=\'%23000000\' fill-opacity=\'1\'%3E%3Cpath d=\'M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z\'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E');"></div>
        
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-8 relative z-10">
            <div class="flex-1">
                <div class="flex flex-wrap items-center gap-3 mb-3">
                    <span class="bg-orange-100 text-orange-600 font-black px-3 py-1.5 rounded-lg text-xs uppercase tracking-widest flex items-center gap-1.5 shadow-sm">
                        <i data-feather="star" class="w-3.5 h-3.5 fill-current"></i> {{ number_format($restaurant->avg_rating, 1) }}
                    </span>
                    <span class="text-sm font-bold text-gray-400 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">{{ $restaurant->total_reviews }}+ Reviews</span>
                    <span class="text-sm font-bold text-gray-400 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100 flex items-center gap-1"><i data-feather="map-pin" class="w-3.5 h-3.5"></i> {{ $restaurant->location }}</span>
                </div>
                <h1 class="text-4xl md:text-5xl font-black text-gray-800 tracking-tight mb-3">{{ $restaurant->name }}</h1>
                ...
                <button class="bg-white border-2 border-gray-200 text-gray-700 font-bold px-6 py-2.5 rounded-xl hover:border-orange-500 hover:text-orange-500 transition-all text-sm shadow-sm flex items-center gap-2 group">
                    <i data-feather="message-square" class="w-4 h-4 text-gray-400 group-hover:text-orange-500 transition-colors"></i> Read User Reviews
                </button>
            </div>
            
            <div class="hidden md:block w-56 h-56 rounded-[2rem] overflow-hidden shadow-2xl border-4 border-white rotate-2 hover:rotate-0 hover:scale-105 transition-all duration-500 flex-shrink-0 bg-gray-100">
                <img src="{{ $restaurant->cover_image ?? 'https://images.unsplash.com/photo-1552566626-52f8b828add9?q=80&w=600&auto=format&fit=crop' }}" class="w-full h-full object-cover">
            </div>
        </div>
    </header>

    <!-- Search & Filter Bar (Sticky) -->
    <div class="sticky top-[68px] sm:top-[76px] z-40 bg-white/80 backdrop-blur-xl shadow-sm border-b border-gray-200/50 py-3 px-6 sm:px-10 lg:px-20 transition-all">
    ...
    <main class="max-w-7xl mx-auto px-6 sm:px-10 lg:px-20 py-12">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8">
            ...
            <div class="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl border border-gray-100 transition-all duration-300 group flex flex-col cursor-pointer" onclick="openItemModal({{ $item->item_id }}, '{{ addslashes($item->item_name) }}', '{{ addslashes($item->description ?? '') }}', {{ $item->price }}, '{{ $item->item_image }}')">
                <div class="relative h-56 overflow-hidden bg-gray-100">
                    <img src="{{ $item->item_image }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                    ...
                    <span class="absolute top-4 left-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold text-gray-700 shadow-sm">{{ $item->category_name }}</span>
                </div>
                <div class="p-6 flex-1 flex flex-col">
                    <h3 class="font-black text-lg text-gray-800 leading-tight mb-2 group-hover:text-orange-500 transition-colors">{{ $item->item_name }}</h3>
                    <p class="text-xs text-gray-400 line-clamp-2 mb-4 font-medium leading-relaxed flex-1">{{ $item->description }}</p>
                    <div class="flex justify-between items-center pt-4 border-t border-gray-50 mt-auto">
                        ...
                        <div class="bg-orange-50 text-orange-500 p-2.5 rounded-xl transition-all shadow-sm group-hover:bg-orange-500 group-hover:text-white">
    ...
    <!-- Overlay Background -->
    <div id="modalOverlay" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] hidden opacity-0 transition-opacity duration-300"></div>

    <!-- 1. Item Detail Modal -->
    <div id="itemModal" class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-md bg-white rounded-[2.5rem] shadow-2xl z-[120] hidden opacity-0 scale-95 transition-all duration-300 overflow-hidden flex flex-col max-h-[90vh]">
        <div class="relative h-64 sm:h-72 bg-gray-100 flex-shrink-0">
            <img id="modalImg" src="" class="w-full h-full object-cover">
            <button onclick="closeModals()" class="absolute top-5 right-5 bg-white/90 backdrop-blur-md p-2.5 rounded-full text-gray-800 hover:bg-white hover:scale-110 shadow-sm transition-all border border-gray-100">
        ...
        <div class="px-8 pb-8 flex-1 overflow-y-auto -mt-6 relative z-10">
            ...
            <p id="modalDesc" class="text-gray-500 font-medium mb-8 text-sm leading-relaxed">Item description goes here.</p>
            
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8 pt-6 border-t border-gray-100">
                <div>
                ...
                <div class="flex items-center bg-gray-50 rounded-2xl p-1.5 border border-gray-200">
                    <button onclick="updateModalQty(-1)" class="w-12 h-12 flex items-center justify-center text-gray-600 hover:bg-white hover:shadow-sm rounded-xl transition-all">
                        <i data-feather="minus" class="w-5 h-5"></i>
                    </button>
                    <span id="modalQty" class="w-12 text-center font-black text-xl text-gray-800">1</span>
                    <button onclick="updateModalQty(1)" class="w-12 h-12 flex items-center justify-center text-orange-500 bg-orange-50 hover:bg-white hover:shadow-sm rounded-xl transition-all border border-orange-100 hover:border-transparent">
                        <i data-feather="plus" class="w-5 h-5"></i>
                    </button>
                </div>
            </div>
            ...
            <!-- Add to Cart Button -->
            <button id="addToCartBtn" class="w-full bg-gray-900 text-white font-black text-base py-4 rounded-2xl shadow-xl hover:bg-black hover:scale-[1.02] transition-all flex justify-between items-center px-6 group">
                <span class="flex items-center gap-2"><i data-feather="shopping-cart" class="w-5 h-5 text-gray-400 group-hover:text-white transition-colors"></i> Add to Cart</span>
                <span id="modalBtnTotal" class="bg-gray-800 px-3 py-1 rounded-lg text-sm group-hover:bg-gray-700 transition-colors">$0.00</span>
            </button>
        </div>
    </div>
    ...
    <!-- 2. Offers Modal -->
    <div id="offersModal" class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-sm bg-white rounded-[2rem] shadow-2xl z-[120] hidden opacity-0 scale-95 transition-all duration-300 p-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl font-black text-gray-800 flex items-center gap-2">
                <i data-feather="percent" class="text-orange-500"></i> Active Deals
            </h2>
            <button onclick="closeModals()" class="p-2 bg-gray-50 border border-gray-100 rounded-full hover:bg-gray-100 transition-colors">
                <i data-feather="x" class="w-4 h-4 text-gray-500"></i>
            </button>
        </div>
        
        <div class="relative overflow-hidden rounded-[1.5rem] bg-gradient-to-br from-orange-400 to-orange-500 p-8 text-center text-white shadow-xl shadow-orange-200 mb-6 border border-orange-300">
            ...
            <p id="offerDesc" class="font-medium text-orange-50 text-sm mb-6 max-w-[200px] mx-auto">Use code PANDA20 on orders above $50.</p>
            <button class="bg-white text-orange-500 font-black px-8 py-3 rounded-xl text-sm hover:scale-105 transition-transform shadow-md uppercase tracking-wider">USE Code</button>
        </div>
    ...
    <script>
        ...
        function addToCart(item) {
            ...
            renderCart();
            openCart();
            setTimeout(closeCart, 2500); // Auto close confirmation
        }
